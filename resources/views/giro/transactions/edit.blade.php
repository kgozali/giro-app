...
<body>
  ...
  <form action="{{ route('edit_giro', $transaction->id) }}" enctype="multipart/form-data" method="POST">
    {{ csrf_field() }}

    <div class="form-group mb-3">
        <label for="giro_code" class="form-label">Kode Giro</label>
        <input name="giro_code" type="text" class="form-control {{ ($errors->has('giro_code') ? 'border-danger' : '') }}" id="giro_code" placeholder="Kode Giro" value="{{ $transaction->giro_code }}">
        @error ('giro_code')
    ...
    <div class="form-group d-grid gap-2 d-md-flex justify-content-md-end">
      <button type="button" class="btn btn-danger btn-lg" onclick="window.location='{{ route('giro_index') }}'">Cancel</button>
